...
then
    ...
    n0=$!
    set +x
elif [ "$NODE_TYPE" = "mempool" ]
then
    set -x
    RUST_LOG="$LOG_LEVEL" target/release/mempool --config=$NODE_CONFIG --initial_block_config=$NODE_INIT_BLOCK --index=$INDEX > mempool_$INDEX.log 2>&1 &
    n0=$!
    set +x
elif [ "$NODE_TYPE" = "miner" ]
then
    set -x
    RUST_LOG="$LOG_LEVEL" target/release/miner --config=$NODE_CONFIG --index=$INDEX --mempool_index=$COMPUTE_INDEX > miner_$INDEX.log 2>&1 &
    n0=$!
    set +x
elif [ "$NODE_TYPE" = "user" ]
then
    set -x
    RUST_LOG="$LOG_LEVEL" target/release/user --config=$NODE_CONFIG --initial_block_config=$NODE_INIT_BLOCK --index=$INDEX --mempool_index=$COMPUTE_INDEX > user_$INDEX.log 2>&1 &
    n0=$!
    set +x
fi
